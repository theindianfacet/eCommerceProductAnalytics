...
#     for col, key in [("utm_source","utm_source"), ("utm_campaign","utm_campaign"),
...
#     df = items.copy()
#     df = df[(df["created_at"] >= F["start_ts"]) & (df["created_at"] <= F["end_ts"])]
#     df = df.merge(products[["product_id","product_name"]], left_on="product_id", right_on="product_id", how="left")
...
#         df["bucket"] = df["date"] - pd.to_timedelta(df["date"].dt.weekday, unit="d")  # Monday week start
#     elif granularity == "Monthly":
#         df["bucket"] = df["date"].values.astype("datetime64[M]")
...
#     pv_counts = pageviews.groupby("website_session_id", dropna=False).size().rename("pv_count").reset_index()
#     sess = sessions.merge(pv_counts, on="website_session_id", how="left").fillna({"pv_count": 0})
#     single = (sess["pv_count"] == 1).sum()
#     total = len(sess)
#     return float(single) / total if total > 0 else np.nan
# 
# def compute_conversion_rate(sessions: pd.DataFrame, orders: pd.DataFrame) -> float:
#     ordered_sessions = orders["website_session_id"].dropna().unique()
#     total_sessions = len(sessions)
#     conversions = len(set(ordered_sessions))
...
#     gross_rev = orders["price_usd"].sum(skipna=True)
#     refund_amt = refunds["refund_amount_usd"].sum(skipna=True) if len(refunds) else 0.0
...
#     g_ordered_sessions = set(orders["website_session_id"].dropna().tolist())
#     g_conv = len(set(g_sessions["website_session_id"]).intersection(g_ordered_sessions)) / len(g_sessions) if len(g_sessions) > 0 else np.nan
...
#     s = sessions.copy()
#     s["hour"] = pd.to_datetime(s["created_at"]).dt.hour
#     s["weekday"] = pd.to_datetime(s["created_at"]).dt.day_name()
